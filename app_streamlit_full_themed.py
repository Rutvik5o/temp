...
if run:
    ...
    with right:
        st.markdown('<div class="info-card"><strong>Retention curve (tenure)</strong></div>', unsafe_allow_html=True)
    ...
    if signup_col != "(none)" and last_col != "(none)":
        try:
            ...
            heatmap = go.Figure(data=go.Heatmap(z=heat_z, x=heat_x, y=heat_y, colorscale="YlGnBu", text=heat_z,
