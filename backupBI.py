...
# ------------------------- HELPERS ---------------------------------------
def apply_chart_layout(fig, rotate_x=False, horizontal=False, height=None):
    """Apply consistent margins and tick/label adjustments to avoid overlap."""
    # default margins (leave room for rotated labels)
    marg = dict(l=60, r=30, t=50, b=120)
    if height is not None:
        fig.update_layout(height=height)
    fig.update_layout(margin=marg)
    try:
        # rotate x-axis labels when requested
        if rotate_x:
            fig.update_xaxes(tickangle=-45, automargin=True)
        else:
            fig.update_xaxes(automargin=True)
        # ensure y-axis labels have room
...
if run:
    ...
    with left_col:
        ...
        if tenure_col != "(none)" and df2[tenure_col].dropna().shape[0] > 0:
            ...
            fig = apply_chart_layout(fig, rotate_x=True)
